---
layout: post
title: "ant编译打包混淆签名Android工程"
summary: "ant编译打包混淆签名Android工程，效率很高，build.xml文件可以自定义"
category: Android
tags: [Android, 工具]
---
{% include JB/setup %}
...
<project>为项目路径   <target>为对应sdk的id  
自动生成Build.xml文件，Build.xml的help改成release或debug修改好之后，再运行build.xml文件，就会执行我们需要的编译过程了。
build.xml文件的执行方法为：右键->Run As->Ant Build  
build.xml文件如下：  
{% highlight java %}
<?xml version="1.0" encoding="UTF-8"?>
<project name="android_rules" default="release">
	<!--自己需要添加的属性 -->
	<property name="sdk.dir" value="D:\\android-sdk-windows" />
		<property name="key.store" value="E:\\Joyuan" />
	<property name="key.alias" value="joyuan" />
	<property name="key.store.password" value="01278767" />
	<property name="key.alias.password" value="01278767" />
...
在工程中添加proguard.cfg文件  
###签名  
准备好key.store文件，在build.xml中设置路径
